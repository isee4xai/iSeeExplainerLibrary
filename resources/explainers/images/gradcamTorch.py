...
class GradCamTorch(Resource):
    ...
    def post(self):
        ...
        if backend!="PYT":
            raise "Only PyTorch models are compatible with this explanation method."
        ...
        if model_file!=None:
            ...
            raise "This method requires a PyTorch model to be uploaded."
        ...
        if len(rgb_img.shape)<3:
            raise "The provided image must be RGB-encoded."
        ...
        if "target_layer" in params_json:
            if hasattr(mlp,params_json["target_layer"]):
                ...
                raise "The specified target layer" + str(params_json["target_layer"]) + "does not exist."
        else:
            raise "This method requires the name of target layer to be provided as a string. This is the layer that you want to compute the visualization for."\
        ...
        if "target_layer_index" in params_json:
            try:
                ...
                raise "The specified index could not be accessed on the target_layer."
