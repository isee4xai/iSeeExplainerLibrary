...
class LimeSegment(Resource):
    ...
    def post(self):
        ...
        #explanation
        exp=LIMESegment(instance, model, model_type=model_type,distance=distance,window_size=window_size, cp=cp)
        ...
        if exp[0][i-1]>=0:
            plt.axvspan(exp[1][i-1], tslen-1, color='green', alpha=(exp[0][i-1]/scale), lw=0)
        else:
            plt.axvspan(exp[1][i-1], tslen-1, color='red', alpha=abs(exp[0][i-1])/scale, lw=0)
    
        plt.plot(range(tslen), instance.flatten(), color='b',lw=1.5)
        plt.xticks(list(plt.xticks()[0]) + [tslen-1])
